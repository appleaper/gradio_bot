...
class MilvusArticleManager:
    ...
    def create_collection(self, collection_name):
        ...
        schema.add_field(field_name="embed_model_name", datatype=DataType.VARCHAR, max_length=512)
        schema.add_field(field_name="platform", datatype=DataType.VARCHAR, max_length=512)
        schema.add_field(field_name="database_type", datatype=DataType.VARCHAR, max_length=256)
        ...
        string_fields = ["article_id", "user_id", "title", "page_count", "file_from", "hash_check", "embed_model_name", "platform", "database_type"]
    ...
    def insert_data_to_milvus(self, df, user_id):
        ...
        for line in data:
            ...
            if type(line['vector']) == list:
                line['vector'] = np.array(line['vector'], dtype=np.float32)
    ...
    def search_vectors_with_articles(self, user_id, emb_model_name, query_vector, articles_id_list, limit=3):
        '''根据文章id列表来查询相似向量'''
        self.check_collections_state(user_id)
        filter_conditions = f"article_id in {articles_id_list} AND embed_model_name == '{emb_model_name}'"
        res = self.client.search(
            collection_name=user_id,
            data=[query_vector],
            limit=limit,
        ...
        for i, hit in enumerate(res[0]):
            output_info_list[i]['score'] = hit['distance']
        df = pd.DataFrame(output_info_list)
        if len(df) != 0:
            df = df[['title', 'content', 'file_from', 'score']]
        return df
    ...
    def delete_data_by_article_id(self, user_id, articles_id_list):
        ...
        self.client.flush(collection_name=user_id)
        gr.Info(f"{res['delete_count']} record has been delete!")
