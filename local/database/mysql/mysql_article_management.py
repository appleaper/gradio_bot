...
class MySQLDatabase:
    ...
    def create_table(self, table_name):
        ...
        try:
            with self.connection.cursor() as cursor:
                columns = ("article_id VARCHAR(255), user_id VARCHAR(255), title VARCHAR(255), content TEXT, "
                           "page_count VARCHAR(255), file_from VARCHAR(255), hash_check VARCHAR(255), "
                           "PRIMARY KEY (hash_check), embed_model_name VARCHAR(255), platform VARCHAR(255), "
    ...
    def insert_data(self, table_name, article_id, user_id, title, content, page_count, file_from, hash_check, embed_model_name, platform, database_type):
        if self.connection is None:
            print("请先连接到数据库")
            return

        values = [article_id, user_id, title, content, page_count, file_from, hash_check, embed_model_name, platform, database_type]
        values = [self._replace_nan(val) for val in values]

        try:
            with self.connection.cursor() as cursor:
                insert_query = (f"INSERT INTO {table_name} (article_id, user_id, title, content, page_count, "
                                f"file_from, hash_check, embed_model_name, platform, database_type) VALUES (%s, %s, "
                                f"%s, %s, %s, %s, %s, %s, %s, %s)")
                # 打印实际执行的插入语句
    ...
    def select_data(self, table_name, user_id, input_text, article_id_list, limit=3):
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                columns_name = ['article_id', 'user_id', 'title', 'content', 'page_count', 'file_from', 'hash_check', 'embed_model_name', 'platform', 'database_type']
    ...
    def insert_data_format_df(self, table_name, df):
        ...
        for index, article_info in tqdm(df.iterrows(), total=len(df)):
            ...
            embed_model_name = self._replace_nan(article_info['embed_model_name'])
            platform = self._replace_nan(article_info['platform'])
            database_type = self._replace_nan(article_info['database_type'])
            check_flag = self.insert_data(
                table_name, article_id, user_id, title, content, page_count, file_from,
                hash_check, embed_model_name, platform, database_type
            )
            if check_flag:
