...
def imgs_rec_predict(img_path, model_name):
    ...
    args.rec_yaml_path = config['rec'][model_name].get('rec_yaml_path', args.rec_yaml_path)
    args.rec_model_path = config['rec'][model_name].get('rec_model_path', args.rec_model_path)
    args.rec_image_shape = config['rec'][model_name].get('rec_image_shape', args.rec_image_shape)
    args.rec_char_dict_path = config['rec'][model_name].get('rec_char_dict_path', args.rec_char_dict_path)
    args.rec_char_type = config['rec'][model_name].get('rec_char_type', args.rec_char_type)
    ...
    for ino in range(len(img_list)):
        for i in rec_res:
            res_word = i[0]
            res_score = i[1]
            result_str += f'word:{res_word}, score:{res_score}\n'
    print("Total predict time for {} images, cost: {:.3f}".format(
        len(img_list), predict_time))
    return result_str, []
