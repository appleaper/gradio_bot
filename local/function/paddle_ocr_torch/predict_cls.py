...
def main(args):
    ...
    for ino in range(len(img_list)):
        angle = cls_res[ino][0]
        score = cls_res[ino][1].tolist()
        msg += f'角度:{angle}, 置信度:{score}'
    print("Total predict time for {} images, cost: {:.3f}".format(
        len(img_list), predict_time))
    return msg

def imgs_cls_predict(img_path, model_path):
    args = utility.iniit_config(image_dir=img_path, cls_model_path=model_path)
    msg = main(args)
    return msg, []


if __name__ == '__main__':
    image_dir = r'C:\use\code\PaddleOCR2Pytorch-main\doc\imgs_words\ch\word_1.jpg'
    cls_model_path = r'C:\use\code\PaddleOCR2Pytorch-main\model\ch_ptocr_mobile_v2.0_cls_infer.pth'
    args = utility.iniit_config(image_dir=image_dir, cls_model_path=cls_model_path)
    msg = main(args)
    print(msg)
