...
class UiDataDeal():
    ...
    def init_default_value_ui(self, request:gr.Request):
        ...
        book_type = gr.Dropdown(choices=list(self.config_dict['article_group_dict'].keys()), value='',label="上下文知识")
    ...
    def sync_variables(self,
        ...
        chat_emb_type = gr.Dropdown(choices=self.config_dict['all_model_list'], value=embedding_types[emb_type_pos], label='编码方式', interactive=True)
        rag_emb_type = gr.Dropdown(choices=self.config_dict['all_model_list'], value=embedding_types[emb_type_pos], label='编码方式', interactive=True)
        kb_emb_type = gr.Dropdown(choices=self.config_dict['all_model_list'], value=embedding_types[emb_type_pos], label='编码方式', interactive=True)
        search_emb_type = gr.Dropdown(choices=self.config_dict['all_model_list'], value=embedding_types[emb_type_pos], label='编码方式', interactive=True)
