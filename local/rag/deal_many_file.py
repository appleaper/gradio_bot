...
def save_rag_group_csv_name(df2, rag_data_csv_dir, id, rag_list_config_path, user_name):
    ...
    df2['database_type'] = akb_conf_class.database_type
...
def deal_mang_knowledge_files(rag_upload_files, is_same_group, knowledge_name, request: gr.Request, progress=gr.Progress()):
    ...
    for file_index, file_name in tqdm(enumerate(rag_upload_files), total=len(rag_upload_files)):
        ...
        if is_same_group == '否' and file_index!=0:
            ...
            id = save_rag_group_name(user_name, article_name, data, akb_conf_class.articles_user_path)
...
def add_group_database(selected_documents_list, new_knowledge_base_name, request: gr.Request):
    ...
    user_name = request.username
    if os.path.exists(akb_conf_class.kb_article_map_path):
        # 若文件存在，加载知识库信息
        all_knowledge_bases_record_with_user = read_json_file(akb_conf_class.kb_article_map_path)
    else:
        raise gr.Error(f'{akb_conf_class.kb_article_map_path} not exist!')
    ...
    if len(new_knowledge_base_name) == 0:
        ...
        # 保存更新后的知识库信息
        save_json_file(all_knowledge_bases_record_with_user, akb_conf_class.kb_article_map_path)
        gr.Info('新建知识库成功')
        return [], '', all_knowledge_bases_record_with_user[user_name], all_knowledge_bases_record_with_user[user_name]

def delete_group_database(knowledge_bases_to_delete, kb_database_type, request: gr.Request):
    # 所有知识库的记录，键为知识库名，值为构成该知识库的文章名列表
    user_name = request.username
    akb_conf_class.get_database_config(kb_database_type)
    if os.path.exists(akb_conf_class.kb_article_map_path):
        # 若文件存在，加载知识库信息
        all_knowledge_bases_record = read_json_file(akb_conf_class.kb_article_map_path)
    else:
        raise gr.Error(f'{akb_conf_class.kb_article_map_path} not exist')
    ...
    # 保存更新后的知识库信息
    save_json_file(all_knowledge_bases_record, akb_conf_class.kb_article_map_path)
    gr.Info('删除知识库成功')
