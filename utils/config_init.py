import os
import torch
from utils.tool import singleton, encrypt_username
    ...
    def get_user_id2article_and_article_group(self, db_name, user_name, embedding_type):
        ...
        if user_name in article_group_dict_info:
            if embedding_type in article_group_dict_info[user_name]:
                ...
                article_group_dict_info[user_name][embedding_type] = {}
                article_group_info = article_group_dict_info[user_name][embedding_type]
            article_group_info['空'] = []
        else:
            article_group_dict_info[user_name] = {}
            article_group_dict_info[user_name][embedding_type] = {}
            article_group_dict_info[user_name][embedding_type]['空']= []
            article_group_info = article_group_dict_info[user_name][embedding_type]

        save_json_file(article_group_dict_info, article_group_path)
